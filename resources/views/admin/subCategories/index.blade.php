...
@section('mainTitle', __('Alt Kategoriler'))
@section('path1', 'Panel')
            ...
            <div class="card mb-5 mb-xl-8">
                ...
                <div class="card-header border-0 pt-5">
                    <h3 class="card-title align-items-start flex-column">
                        <span class="card-label fw-bolder fs-3 mb-1">{{__('Alt Kategoriler')}}</span>
                        <span class="text-muted mt-1 fw-bold fs-7">{{__('Toplam alt kategori sayısı')}} {{ $subcategories->count() }} </span>
                    </h3>
                        ...
                        <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
                            ...
                            <thead>
                                <tr class="fw-bolder text-muted">
                                    ...
                                    <th class="min-w-140px">{{__('Alt Kategori Adı')}}</th>
                                    <th class="min-w-140px">
                                        @if (isset($order))
                                            @if ($order[0] == 'name')
                                                @if ($order[1] == 'asc')
                                                    <a href="{{route('subcategory.sort',['order'=>'name,desc'])}}">{{__('Alt Kategori Adı')}}
                                                ...
                                                @else
                                                    <a href="{{route('subcategory.sort',['order'=>'name,asc'])}}">{{__('Alt Kategori Adı')}}
                                                    <span class="svg-icon svg-icon-primary svg-icon-2hx"><svg style="height: 1.5rem !important;width:1.5rem !important" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                                ...
                                                @endif
                                            @else
                                            <a class="text-muted" href="{{route('subcategory.sort',['order'=>'name,asc'])}}">{{__('Alt Kategori Adı')}}</a> 
                                            @endif
                                        @else
                                            <a class="text-muted" href="{{route('subcategory.sort',['order'=>'name,asc'])}}">{{__('Alt Kategori Adı')}}</a> 
                                        @endif 
                                    </th>
                                    {{-- <th class="min-w-140px">Açıklaması</th> --}}
                            ...
                            <tbody>
                                @foreach ($subcategories as $category)
                                    <tr>
                                        ...
                                        <td>
                                            {{ getCategory(['id' => $category->category_id])->name }}
                                        </td>
                                        <td>
                                            {{ $category->name }}
                                        </td>
                                            ...
                                            <div class="d-flex justify-content-end flex-shrink-0">
                                                <div class="d-flex justify-content-end flex-shrink-0">
                                                    <a class="btn btn-sm  {{ $category->status == '1' ? 'btn-light-success' : 'btn-light' }}  me-3"
                                                        href="{{ route('subcategory.destroy', ['id' => $category->id]) }}">
